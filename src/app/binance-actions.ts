...
export async function getBinanceBalances() {
  try {
    ...
    const wallets: { wallet: string; assets: WalletAsset[]; totalUsd: number }[] = [];

    // Process Spot
    if (!('error' in spot)) {
      const spotAssets: WalletAsset[] = spot.map((a: { asset: string; free: string; locked: string }) => {
      ...
      wallets.push({ wallet: "Spot", assets: spotAssets, totalUsd: spotAssets.reduce((sum: number, a: WalletAsset) => sum + a.usdValue, 0) });
    }

    // Process Funding
    if (!('error' in funding)) {
      const fundingAssets: WalletAsset[] = funding.map((a: { asset: string; free: string }) => {
        const totalAmount = parseFloat(a.free);
        return { asset: a.asset, totalAmount: totalAmount.toString(), usdValue: calculateUsdValue(a.asset, totalAmount, prices) }
      });
      wallets.push({ wallet: "Funding", assets: fundingAssets, totalUsd: fundingAssets.reduce((sum: number, a: WalletAsset) => sum + a.usdValue, 0) });
    }
